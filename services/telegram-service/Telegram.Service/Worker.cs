...
public sealed class Worker : BackgroundService
{
    ...
    {
        using var consumer = _consumerFactory.Create("telegram-service");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                if (processedEvent is null || processedEvent.Channel != DispatchChannel.Telegram)
                {
                ...
                using (_logger.BeginScope(new Dictionary<string, object> { ["eventId"] = processedEvent.EventId, ["deviceId"] = processedEvent.DeviceId }))
                {
                    _logger.LogInformation(
                        "Telegram mock dispatch for user {UserId} device {DeviceId} message {Message}",
                    ...
                    var dispatchResult = new AlertDispatchResultEvent(
                        ...
                        Channel: DispatchChannel.Telegram,
                    ...
                    await _publisher.PublishAsync(
                        ...
                        $"{processedEvent.EventId}:{processedEvent.DeviceId}:telegram",
                        dispatchResult,
                        stoppingToken);
                }
            ...
            {
                _logger.LogError(exception, "Kafka consume error in telegram service");
            }
            catch (Exception exception)
            {
                _logger.LogError(exception, "Unexpected error in telegram service");
